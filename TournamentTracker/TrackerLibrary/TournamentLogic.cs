...
namespace TrackerLibrary
{
    public static class TournamentLogic
    {
        ...
        private static void CompleteTournament(TournamentModel model)
        {
            ...
            TeamModel winners = model.Rounds.Last().First().Winner;                                                                         // winner of the last round

            TeamModel runnerUp = model.Rounds.Last().First().Entries.Where(x => x.TeamCompeting != winners).First().TeamCompeting;          // takes the person who goes to the
                                                                                                                                            // last round and is not the winner

            decimal winnerPrize = 0;
            decimal runnerUpPrize = 0;

            // Counts the money from prizes
            if (model.Prizes.Count > 0)
            {
                decimal totalIncome = model.EnteredTeams.Count * model.EntryFee;

                PrizeModel firstPlacePrize = model.Prizes.Where(x => x.PlaceNumber == 1).FirstOrDefault();                                  // First or Default - find first value
        ...
        private static void CreateOtherRounds(TournamentModel model, int rounds)
        {
            ...
            while (round <= rounds)
            {
                foreach (MatchupModel match in previousRound)
                {
                    ...
                    if (currMatchup.Entries.Count > 1)                                                  // > 1 makes sure that code capture 2, but
                                                                                                        // every reason code got beyond 2, its ok
                    {
        ...
        {
            return teams.OrderBy(x => Guid.NewGuid()).ToList();                                             //guid is a unique indentifier, a random 16 byte character value
        }
